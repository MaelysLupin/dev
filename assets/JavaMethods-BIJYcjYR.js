import{au as e,c as s,e as n,a as o}from"./index-DCMV4SLN.js";const a={},r={className:"card p-4"};function i(l,t){return o(),s("div",r,t[0]||(t[0]=[n(`<div class="flex items-center mb-4"><i class="pi pi-fw pi-cog text-purple-500 mr-2"></i><h1 class="font-semibold text-xl">Méthodes &amp; Tableaux</h1></div><h2 class="font-semibold text-lg mt-4 mb-2">Définir une méthode</h2><pre class="bg-gray-100 p-3 rounded-lg text-sm"><code>public static int addition(int a, int b) {
    return a + b;
}</code></pre><h2 class="font-semibold text-lg mt-4 mb-2">Surcharge de méthodes</h2><pre class="bg-gray-100 p-3 rounded-lg text-sm"><code>public static int addition(int a, int b, int c) {
    return a + b + c;
}</code></pre><h2 class="font-semibold text-lg mt-4 mb-2">Tableaux</h2><pre class="bg-gray-100 p-3 rounded-lg text-sm"><code>int[] nombres = {1, 2, 3};
System.out.println(nombres[0]); // 1</code></pre><h2 class="font-semibold text-lg mt-4 mb-2">Collections</h2><p class="mb-2 font-medium">Liste (ArrayList) :</p><pre class="bg-gray-100 p-3 rounded-lg text-sm"><code>import java.util.*;

List&lt;String&gt; liste = new ArrayList&lt;&gt;();
liste.add(&quot;Java&quot;);
liste.add(&quot;Python&quot;);

for (String s : liste) {
    System.out.println(s);
}</code></pre><p class="mb-2 font-medium">Map (clé/valeur) :</p><pre class="bg-gray-100 p-3 rounded-lg text-sm"><code>Map&lt;String, Integer&gt; map = new HashMap&lt;&gt;();
map.put(&quot;Alice&quot;, 25);
map.put(&quot;Bob&quot;, 30);

System.out.println(map.get(&quot;Alice&quot;)); // 25</code></pre>`,12)]))}const d=e(a,[["render",i]]);export{d as default};
